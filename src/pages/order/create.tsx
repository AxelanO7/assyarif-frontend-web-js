...
import { StockProps } from "@/types/stuff";
...
const CreateOrder = () => {
  ...
  const [selectedStock, setSelectedStock] = useState<StockProps>();
  ...
  const getStocks = () => {
    axios
      ...
      .then((res) => {
        console.log(res.data);
        const dataRes: StockProps[] = res.data.data;
        setStocks(dataRes);
      })
      .catch((err) => {
        console.error(err);
  ...
  const handleSelectStuff = (stock: StockProps) => {
